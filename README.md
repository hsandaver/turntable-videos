...
A GLB from the GLB texture processor comes before all of these. The processor writes `GLB texture processor` into each GLB it makes, and when it updates a ZIP it puts its GLB at the original's path, such as `bundle-medium.glb`. The script checks for that mark, so a brightened copy gets rendered instead of the darker OBJ beside it. The processor also records how many stops it brightened the textures, and the app shows that in the list, as in `bundle-medium.glb (processed, +2 stops)`. GLBs from versions of the processor that didn't record brightening show just *processed*. Any processed GLB counts, whether or not it was brightened.
